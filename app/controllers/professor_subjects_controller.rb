class ProfessorSubjectsController < ApplicationController
  ...
  def show
    ...
    @rating = if user_signed_in?
                ProfessorSubjectUser.where(user_id: current_user.id).find_by(professor_subject_id: @professor_subject.id)
              else
                false
              end
  ...
  def update_user_professor_subject_rating
    ...
    @rating.trabalhos = params[:trabalhos] if params[:trabalhos]
    @rating.provas = params[:provas] if params[:provas]
    @rating.tarefas = params[:tarefas] if params[:tarefas]
